...
<div class="form-container">
    ...
    <div style="margin-bottom: 24px;">
        <nav aria-label="breadcrumb" style="margin-bottom: 12px; font-size: 13.5px;">
            <ol style="list-style: none; padding: 0; margin: 0; display: flex; align-items: center; gap: 8px;">
                <li>
                    <a href="{{ route('pegawai.index') }}" style="color: var(--n-500); text-decoration: none; font-weight: 500;">Kelola Pegawai</a>
                </li>
    ...
    <div class="card form-page">
        <form method="POST" action="{{ route('pegawai.update', $pegawai) }}" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="card-body" style="display: flex; flex-direction: column; gap: 20px;">
                ...
                <div class="form-group">
                    <label for="nik" class="form-label">
                        ...
                        <span style="font-size:11px; color:var(--n-400); font-weight:400; margin-left: 4px;">(16 digit angka)</span>
                    </label>
                    <input type="text" id="nik" name="nik" value="{{ old('nik', $pegawai->nik) }}" class="form-input" style="font-family: monospace; letter-spacing: .5px;" placeholder="0000000000000000" minlength="16" maxlength="16" pattern="\d{16}" required>
                    @error('nik')<div class="form-error">{{ $message }}</div>@enderror
                </div>

                <div class="form-group">
                    <label for="nip" class="form-label">
                        NIP (Nomor Induk Pegawai) <span style="font-size:11px; color:var(--n-400); font-weight:400; margin-left: 4px;">(Opsional, 18 digit angka)</span>
                    </label>
                    <input type="text" id="nip" name="nip" value="{{ old('nip', $pegawai->nip) }}" class="form-input" style="font-family: monospace; letter-spacing: .5px;" placeholder="Opsional">
...
document.addEventListener('DOMContentLoaded', function () {
    const apiBase    = '{{ rtrim(route("api.units.sub-units", ["unit" => "__UNIT__"]), "") }}';
