...
@section('title', 'Edit Kategori ' . (auth()->user()->role->nama_role === 'admin' ? 'Admin' : 'Operator') . ' — SIKANDIS')
    ...
    <div style="margin-bottom: 24px;">
        <nav aria-label="breadcrumb" style="margin-bottom: 12px; font-size: 13.5px;">
            <ol style="list-style: none; padding: 0; margin: 0; display: flex; align-items: center; gap: 8px;">
                <li>
                    <a href="{{ route('kategori.index') }}" style="color: var(--n-500); text-decoration: none; font-weight: 500;">Kelola Kategori</a>
                </li>
                <li style="color: var(--n-400);">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>
                </li>
                <li style="color: var(--n-900); font-weight: 600;" aria-current="page">{{ $kategori->nama_kategori }}</li>
            </ol>
        </nav>
        <h2 class="page-heading">Edit Kategori Kendaraan</h2>
    </div>
        ...
        <form method="POST" action="{{ route('kategori.update', $kategori) }}" novalidate>
            ...
            <div class="card-body">
                <div class="form-group" style="margin-bottom: 0;">
                    ...
                    <input type="text" id="nama_kategori" name="nama_kategori" value="{{ old('nama_kategori', $kategori->nama_kategori) }}" class="form-input" placeholder="Contoh: Roda Empat" autofocus>
